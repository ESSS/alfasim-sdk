from alfasim_sdk import (
    read_metadata, 
    RESULTS_FOLDER_NAME, 
    get_time_set_and_trend_from_trend_id,
    get_profile_from_profile_id,
)
...
def test_read_results(datadir):
    ...
    total_time_steps = sum(
        (info.size for base_ts, info in time_set_info.items() if base_ts in time_set_key), 0,
    )
    last_ts_index = total_time_steps - 1
        
    profile = get_profile_from_profile_id(profile_id, results_metadata, timestep_id=last_ts_index)