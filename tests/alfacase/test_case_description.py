...
def test_check_profile_description(default_case):
    ...
    assert isinstance(profile_2.length_and_elevation.length, Array)
    assert profile_2.length_and_elevation.length.GetValues("m") == []
    assert isinstance(profile_2.length_and_elevation.elevation, Array)
...
def test_obtain_curve_from_arrays() -> None:
    ...
    curve_2 = obtain_curve_from_arrays(domain=my_array_1, image=my_array_2)
    assert isinstance(curve_2, CurveExpression)

    curve_3 = obtain_curve_from_arrays(domain=my_array_2, image=my_array_1)
    assert isinstance(curve_3, CurveExpression)

    curve_4 = obtain_curve_from_arrays(domain=my_array_2, image=my_array_2)
    assert isinstance(curve_4, CurveExpression)
