...
from alfasim_sdk.types import Tab, Tabs, Group
    ...
    def apply(class_: type):
        ...
        def wrap_class(class_: type):
            ...
            for value in attr.fields(attr_class):
                if not (isinstance(value.default, type) and issubclass(value.default, Tab)):
                    raise TypeError(f"Error on attribute '{value.name}' expecting a class decorated with @tab but received a {value.default.__class__.__name__} type")
...
def group(*, caption: str) -> Callable:
    ...
    def apply(class_: type):
        @functools.wraps(class_)
        def wrap_class(class_: type, caption: str):
            return get_attr_class(class_, caption, icon=None, model=None, bases=(Group,))
