...
class Results:
    ...
    def get_global_trend_curve(self, property_name: str) -> Curve:
        ...
        msg = [
            ...
            *self.list_global_trends(),
        ]
        raise RuntimeError("\n- ".join(msg))

    def list_positional_trends(self) -> Sequence[str]:
        """
        List the collected positional trends.
        """
        metadata = self.metadata
        output_template = "{property_id}@{network_element_name}({position} [m])"
        return [
            output_template.format(**trend_metadata)
            for trend_metadata in metadata.trends.values()
            if "position" in trend_metadata
        ]

    def list_overall_trends(self) -> Sequence[str]:
        """
        List the collected overall trends.
        """
        metadata = self.metadata
        output_template = "{property_id}@{network_element_name}"
        return [
            output_template.format(**trend_metadata)
            for trend_metadata in metadata.trends.values()
            if (
    ...
    def list_global_trends(self) -> Sequence[str]:
        """
        List the collected global trends.
        """
        metadata = self.metadata
        output_template = "{property_id}"
        return [
            output_template.format(**trend_metadata)
            for trend_metadata in metadata.trends.values()
    ...
    def list_profiles(self) -> Sequence[str]:
        ...
        metadata = self.metadata
        output_template = (
            "{property_id}@{network_element_name}(timesteps={time_set_size})"
        )

        def format_profile_caption(meta: Dict) -> str:
            ...
            return output_template.format(time_set_size=time_set_size, **meta)

        return [
            format_profile_caption(profile_metadata)
